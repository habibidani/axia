                    ...
                    @if ($company && $company->name)
                        ...
                        <div class="mt-1 flex items-center gap-3 text-sm text-gray-600">
                            ...
                            @if ($company->team_cofounders || $company->team_employees)
                                <span>•</span>
                                <span>{{ $company->team_cofounders ?? 0 }} founders ·
                                    {{ $company->team_employees ?? 0 }} employees</span>
                            @endif
                        </div>
                    @else
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                    ...
                    <div class="space-y-3 mb-6">
                        <div class="flex items-start gap-3">
                            <div
                                class="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-rose-500 to-pink-500 rounded-full flex items-center justify-center text-white text-sm font-semibold">
                                A
                            </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 bg-gray-50 rounded-xl px-4 py-3">
                                <p class="text-sm text-gray-700">Drop your to-dos here and I'll show you what really
                                    matters.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Paste your tasks for this week. axia will analyze them
                                against your goals and KPIs.</p>
                        </div>
                        ...
                        <div class="space-y-4 text-sm text-gray-700">
                            <div>
                                ...
                                <ul class="mt-1 ml-4 list-disc space-y-1 text-gray-600">
                                    <li><strong>60%</strong> - Top KPI Impact: "Does this directly move
                                        {{ $topKpi?->name ?? 'your top metric' }}?"</li>
                                    <li><strong>30%</strong> - Goal Alignment: "Which high-priority goal does this
                                        serve?"</li>
                                    <li><strong>10%</strong> - Urgency & Founder-Level: "Is it blocking? Can only you do
                                        this?"</li>
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul class="mt-1 ml-4 list-disc space-y-1 text-gray-600">
                                    ...
                                    <li>Your <strong>Company Profile</strong> (stage, team, customer profile, market
                                        insights)</li>
                                </ul>
                            </div>
                            ...
                            <div class="pt-3 border-t border-rose-200">
                                <p class="text-xs text-gray-600">
                                    ...
                                    <a href="{{ route('admin.prompts') }}" wire:navigate
                                        class="text-rose-600 hover:text-rose-700 font-medium">Edit System Prompts →</a>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    @if (!$showCsvUpload)
                        <form wire:submit.prevent="analyzeTodos">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach (\App\Services\ExampleContentService::getTodoExamples() as $index => $example)
                                        <button type="button" wire:click="insertExample({{ $index }})"
                            ...
                            <textarea wire:model="todoText" rows="12"
...
Update investor deck"
                                wire:loading.attr="disabled" wire:target="analyzeTodos"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-rose-500 focus:border-transparent resize-none text-sm disabled:opacity-50 disabled:bg-gray-50"></textarea>

                            <div class="mt-6 flex flex-col sm:flex-row gap-3">
                                <button type="submit" wire:loading.attr="disabled" wire:target="analyzeTodos"
                                    ...
                                    <span wire:loading wire:target="analyzeTodos"
                                ...
                                <button type="button" wire:click="$set('showCsvUpload', true)"
                    ...
                    @else
                        <form wire:submit.prevent="uploadCsv">
                            <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center">
                                ...
                                @if ($csvFile)
                                    <p class="mt-2 text-sm text-green-600">Selected:
                            ...
                            <div class="mt-6 flex gap-3">
                                ...
                                <button type="button" wire:click="$set('showCsvUpload', false)"
                                    class="px-6 py-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50">
                                    Use text instead
                                </button>
                            </div>
                ...
                @if ($topKpi)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <span
                                class="inline-flex items-center px-2 py-1 rounded-md bg-rose-50 text-rose-700 text-xs font-medium">
                                Primary
                            </span>
                        </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-baseline">
                                ...
                                <span class="text-xl font-bold text-gray-900">
                                    {{ number_format($topKpi->current_value, 0) }} <span
                                        class="text-sm font-normal text-gray-600">{{ $topKpi->unit }}</span>
                                </span>
                            </div>
                            <div class="flex justify-between items-baseline">
                                <span class="text-sm text-gray-600">Target</span>
                                <span class="text-xl font-bold text-rose-600">
                                    {{ number_format($topKpi->target_value, 0) }} <span
                                        class="text-sm font-normal text-gray-600">{{ $topKpi->unit }}</span>
                                </span>
                            </div>
                        </div>
